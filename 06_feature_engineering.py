...
# For first model version, I use Top 50 only.
# Later I can expand to all 651 forecastable parts.
...
# For forecasting, missing part-months should usually mean zero demand,
# not "row does not exist".
...
# Filling missing numeric demand values with 0.
...
# Fill missing descriptive columns using known part information.
part_info = (
    monthly_part_demand.groupby("Part Number", as_index=False)
    .agg(
        description=("description", "first"),
...
# These help the model understand that December and January are close.
...
#  CREATEING LAG FEATURES -=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-

# lag_1 = previous month demand
# lag_2 = demand two months ago
# lag_3 = demand three months ago
# These are safe because they use only past information.
...
# Important:
# We shift first, then roll.
# This avoids data leakage.
#
# Bad:
# rolling mean includes current month
#
# Good:
# shift(1) means rolling average only uses previous months.
...
#  CREATEING RETURN FEATURES -=-=--=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

# return_ratio tells how much of the sold quantity was returned.
# We use current historical row for analysis, but for forecasting future
# we must use lagged return ratio instead.
...
# I keep outliers, but flag very high demand months.
high_demand_threshold = model_data["net_qty"].quantile(0.99)

model_data["is_high_demand_month"] = (
...
# First months have missing lags because no previous months exist.
# We remove rows where required historical features are missing.
required_features = [
    "lag_1",
    "lag_2",
    "lag_3",
...
# rolling_std_3 can be missing or zero for stable demand.
...
# Older months train the model; newer months test the model.
...
output_file = "06_feature_engineering_report.xlsx"
